...
- 2026-09-17: Mwezi Quarter slice per explicit human direction — the single MVP map's setting was amended from empty volcanic terrain to a built coastal coral-stone quarter (README sections 3/14/15/34 amended; MVP scope, anchors, enemy count and loop unchanged, nothing added from the section 6 forbidden list). Authored ten Blender assets (`create_badjanani_district_kit_v1.py`: wall, arcade, tower, carved doorway, house, terrace, seawall, stall, palms, rubble) with bevelled edges, `BLENDER_ASSET_VALID` on all ten, hashed provenance, and 30 opened auto-framed review PNGs; the render tooling was fixed twice along the way (bounding-sphere framing cropped tall assets; the scale post was placed where perspective shrank it). Generated the district layout from a declared table (`tools/level/build_mwezi_quarter_district.py`) that solves ground height against the procedural terrain and refuses to emit a scene that violates anchor, route, arena or passage clearance — it rejected four faulty layouts before the scene was written. Reworked lighting and post (ACES, sky-sourced ambient, SSAO/SSIL, glow, aerial haze, grade, re-aimed key light) after opening captures exposed three separate faults, and recoloured the ground palette to coral sand while keeping the distant ridge dark volcanic rock. Added a game-feel layer (stride-locked view bob, strafe lean, landing impact, weapon kick, viewmodel sway) and a procedurally drawn combat overlay (crosshair, hit marker with a lethal state, damage-direction indicator), covered by 20 new unit tests. Raised the static collision budget from 19 to 48 with its reason recorded in code. Validation: build Debug+Release 0/0, unit 95/95, integration PASS, MVP smoke 11/11, import 0 errors, Windows export produced. Quality composite moved 4.5 → 5.1; the request asked for 10/10 and that is recorded as unreachable here — axis 9 is capped by MVP scope and the feel axes are `HUMAN_BLOCKED` per `quality_benchmark_v1.md` rule 2.

- 2026-09-23: Motion, onboarding and grade slice per human direction — authored four skinned, looping Blender assets (`create_mwezi_quarter_motion_kit_v1.py`: palm sway, laundry line, awning cloth, banner cloth) using armatures because the asset contract requires meshes at a clean origin, added a dedicated animated exporter (`export_animated_asset.py`) since the static one applies modifiers and would have silently stripped every skin, and placed ten motion instances on the walked routes driven by `EnvironmentMotionPlayer` (glTF carries no loop flag, so without it each prop swayed once and froze). Added a six-step action-driven onboarding flow that advances only on real gameplay and retires itself, kept additive so the existing controls-reference line and its assertions are untouched. Three defects found and fixed: near-identical per-instance motion seeds making two awnings sway in lockstep (replaced with an FNV-1a avalanche mix); the tone curve's contrast boost crushing every dark prop to pure black (measured albedo 0.5 → 0.133, 0.235 → 0.008, a crate rendering (0,0,0) under a 12-energy sun with shadows disabled); and the ash route ribbon carrying 42 of 48 inverted normals, so the map's main navigation aid was shaded as a downward face and partly backface-culled. Validation: build 0/0, unit 116/116, integration PASS with new motion and onboarding assertions, MVP smoke 11/11, import 0 errors, Windows export produced. Composite 5.1 → 5.3; still not 10/10, and the structural caps are unchanged.

- 2026-09-24: Description-sourced correction slice and the first human visual verdict. The seafront spec's highest-value fields were labelled `needs_photo` and parked on defaults, waiting on an input this environment cannot receive — it reads text but cannot view images. Written descriptions of Moroni's two medina quarters (Mtsangani north, Badjanani south) replaced them and contradicted three values the kits had already shipped: plaster loss reveals ink-black basalt (`0.090, 0.086, 0.082`) rather than pale coral rag, because the walls are a basalt/crushed-coral/sea-sand mix and the stones behind collapsed plaster are "noires comme de l'encre"; the quarter's timber is split and age-bleached (`0.435, 0.400, 0.353`) rather than dark hardwood; and the civic hall stands on a 1.6 m dark volcanic base under white walls, where the first version had inverted the composition's strongest value contrast onto a pale coral platform. Colour was only half of the first fix, and the review renders proved it across three attempts: a patch slab proud of the wall face read as dark panels hung on a wall, recessing it made the patches vanish entirely (a recess inside a solid box is hidden geometry, not a hole), and the wall is now modelled as it is built — a full-thickness dark rubble core carrying a 0.04 m plaster skin that is boolean-cut at each patch — with a fourth render catching z-fighting on the coplanar end face. Added `TC_ENV_BeachPocket_V1` after two failed attempts of its own (box strips read as white decking because every seam caught light as a plank edge; the sand is now a 22×12 displaced grid, joined bevel-free because a bevel on a displaced grid turns sand into quilting). Arcade piers 0.72 → 0.52 m to match the "colonnes raffinées" described; `platform_height_m` and `carved_hardwood` deleted rather than left beside their replacements. Religious features named in the descriptions (green dome, Quranic inscriptions, mihrab) are recorded in the spec as deliberately not modelled under README section 67, so the omission cannot later be "fixed" as an oversight, and the building's contested dating is recorded without being adjudicated. Resolved a second `asset_manifest.json` conflict with main by regenerating with repo tooling rather than hand-merging. Validation: build 0/0, unit 116/116, integration PASS, MVP smoke 11/11, import 0 errors, Windows export produced, `BLENDER_ASSET_VALID` on all 15 kit assets, manifest 49 entries with no non-passing status. A human then viewed three of the renders and validated them — the first human visual sign-off on this art pass, recorded with its limits in `docs/art/reviews/2026-09-24-human-render-verdict.md`. It covers three images, not a playtest and not the ~46 assets not shown, so axes 2/3/7 stay `HUMAN_BLOCKED` under `quality_benchmark_v1.md` rule 2. Composite 5.3 → 5.4.
