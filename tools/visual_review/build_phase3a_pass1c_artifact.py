...
REQUIRED = [*PRODUCTION, *DIAGNOSTIC, 'pass1c_production_contact_sheet.png', 'pass1c_diagnostic_contact_sheet.png', 'runtime-contract-report.json', 'terrain-generation-report.json', 'capture.log', 'test-summary.txt']
...
def contact(out: Path, names: list[str], output: str, rows: int) -> None:
...
def main():
    out=Path(sys.argv[1]) if len(sys.argv)>1 else Path('artifacts/visual-review/phase3a-pass1c-directed-terrain')
    contact(out, PRODUCTION, 'pass1c_production_contact_sheet.png', 3)
    contact(out, DIAGNOSTIC, 'pass1c_diagnostic_contact_sheet.png', 2)
    (out/'test-summary.txt').write_text('\n'.join([
        'Phase 3A Pass 1C directed terrain artifact summary',
    ...
    manifest={
        ...
        'capture_command': os.environ.get('CAPTURE_COMMAND','xvfb-run -a godot --path . --script tools/visual_review/capture_phase3a_pass1c_directed_terrain.gd'),
        ...
        'production_filenames': PRODUCTION,
        'diagnostic_filenames': DIAGNOSTIC,
        'filenames':[f['filename'] for f in files]+['manifest.json'],
        'files':files,
        'artifact_name':os.environ.get('ARTIFACT_NAME','phase3a-pass1c-directed-terrain-local')
    }
    (out/'manifest.json').write_text(json.dumps(manifest,indent=2)+'\n')
    print(f'PASS1C_ARTIFACT_MANIFEST_OK {out / "manifest.json"}')
